...
function getPersonlGalleryAddress() {
    var json ="{'addressForm':{'address':'1'}}";
    var jsonObj;
    var Strat;
    var End;
    var Allpage;
    $.ajax({
        ...
        success : function(data) {
            ...
            anAlbumInfo();
            //showPhoto方式
            if( jsonObj.length % 11 == 0 ) Allpage = jsonObj.length/11;
            else Allpage = jsonObj.length/11+1;
            ...
            Strat = (MyPage-1)*11;
            End = MyPage*11;
            if( End >= jsonObj.length ) End = jsonObj.length;
            theWayShowPhoto(jsonObj,Strat,End);
        },
        error: function (data) {
            swal("", "系统出错！", "error");
        }
...
function anAlbumInfo() {
    //var img="images/PhotoGalery/1.jpg";
    var jsonObj;
    $.ajax({
        type : "post",
        url : "/PhotoShow/album_anAlbumInfo.action",
        success : function(data) {
            jsonObj=eval("("+data+")");
